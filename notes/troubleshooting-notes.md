# Troubleshooting Notes — Lab 69 Forfiles Abuse Investigation
...
## 12. Important Investigation Lessons

### Do not treat forfiles.exe as automatically malicious

`forfiles.exe` is a legitimate Windows utility. Its presence alone is weak evidence.

### Analyze the command line

The `/C` parameter is particularly important because it determines what command is executed for matching files.

### Validate parent-child relationships

The expected process chain should be confirmed through telemetry rather than assumed.

### Correlate process and file evidence

The investigation became stronger when the `cmd.exe` command line was correlated with the resulting output file.

### Treat missing telemetry carefully

If a particular Sysmon or Wazuh event is unavailable, this should be recorded as a visibility limitation rather than interpreted as proof that the activity did not occur.

## 13. Final Troubleshooting Assessment

No blocking execution issue prevented the laboratory from being completed.

The main investigative considerations were understanding the execution behavior of `forfiles.exe`, validating the child `cmd.exe` process through Sysmon/Wazuh, and correlating the command with the resulting file.

The final telemetry provided enough evidence to reconstruct the controlled execution chain successfully.
